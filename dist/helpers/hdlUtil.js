...
var oType = function oType(o) {
  return o === null ? "null" : (0, _typeof2["default"])(o);
};
...
var _validateDateStringExpArr = [/^\d{4}[/-](0?[1-9]|1[012])[/-](0?[1-9]|[12][0-9]|3[01])$/ // format: yyyy-mm-dd //  /^(0?[1-9]|[12][0-9]|3[01])[\/\-](0?[1-9]|1[012])[\/\-]\d{4}$/; // format: dd/mm/yyyy
...
var getDatetimeFlag = function getDatetimeFlag(str) {
  var i,
      len,
      regExp,
      match,
  ...
  switch (i) {
    ...
    case 4:
      rtnFlag = 'yyyy-mm-dd hh:mi:ss.fff';
      break;
  }
...
var isDateHhMiSsFffAllZero = function isDateHhMiSsFffAllZero(date) {
...
var putOffTomorrow = function putOffTomorrow(date) {
  if (!(date instanceof Date)) {
    return null;
  }
...
var putOffYesterday = function putOffYesterday(date) {
  if (!(date instanceof Date)) {
    return null;
  }
...
var parseDatetimeStrByFlag = function parseDatetimeStrByFlag(str) {
  ...
  var startPlus = str.indexOf('+');
  var endPlus = str.lastIndexOf('+');
...
var isTimeIntervalValid = function isTimeIntervalValid(_ref) {
  var reqParam = _ref.reqParam,
      _ref$fields = _ref.fields,
      fields = _ref$fields === void 0 ? [] : _ref$fields,
      _ref$unit = _ref.unit,
      unit = _ref$unit === void 0 ? 'day' : _ref$unit,
      _ref$max = _ref.max,
      max = _ref$max === void 0 ? 184 : _ref$max;
  var rtn = {};
  var startField = fields[0];
  var endField = fields[1];
  ...
  var startDateStr = reqParam[startField];
  var endDateStr = endField ? reqParam[endField] : '';
  var startDate = startDateStr ? parseDatetimeStrByFlag(startDateStr) : null;
  var endDate = endDateStr ? parseDatetimeStrByFlag(endDateStr) : new Date();
  ...
  if (!startDate) {
    ...
    var timeGap = getTimeGap(startDate, endDate, unit);
...
var validateDateString = function validateDateString(str) {
  if (!str) {
    return null;
  }
...
var is_yyyymmdd = function is_yyyymmdd(str) {
  if (!str) {
    return null;
  }
...
var parse_yyyymmdd = function parse_yyyymmdd(str) {
  if (oType(str) !== 'string') {
    return str;
  }
...
var paddingZero1 = function paddingZero1(dd) {
  if (oType(dd) === 'number') {
    dd = String(dd);
...
var paddingZero = function paddingZero(dd, length) {
  if (oType(dd) === 'number') {
    dd = String(dd);
  } else if (oType(dd) !== 'string') {
    return;
...
function convertDate2Num(now) {
  var precise = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  ...
  var nowStr = date2string(now, precise);
  var numStr = nowStr.replace(/-|\s|:|\./g, '');
  return numStr;
}
...
function convertNum2DateStr(str) {
  ...
  var partLength = [4, 2, 2, 2, 2, 2];
  var sgnArr = ['-', '-', ' ', ':', ':'];
  var results = [];

  for (var i = 0, skip = 0; i < partLength.length; i++) {
    var pl = partLength[i];
    var sgn = i < sgnArr.length ? sgnArr[i] : '';
...
var date2string = function date2string(date, precise) {
  ...
  if (precise === 'GMT') {
    ...
    var utcMillisecond = date.getUTCMilliseconds();
    var tmpArr1 = [];
    var tmpArr2 = [];
...
var iterateObject4Date2string = function iterateObject4Date2string(obj, precise, fieldsObj) {
  if (oType(obj) !== 'object') {
    return;
  }
...
var iterateObject4certainArray = function iterateObject4certainArray(obj, keyName, resultArr) {
  if (oType(obj) !== 'object') {
    return;
  }
  ...
  Object.keys(obj).forEach(function (key) {
    ...
    if (elem instanceof Array && key === keyName) {
      OUTTER: for (var i = 0; i < elem.length; i++) {
        var subElem = elem[i];
...
var iterateObject4certainObject = function iterateObject4certainObject(obj, validateHandler, resultArr) {
  if (oType(obj) !== 'object') {
    return;
  }
...
var iterateObject4certainKeyVals = function iterateObject4certainKeyVals(_ref2) {
  var obj = _ref2.obj,
      keys = _ref2.keys,
      resultArr = _ref2.resultArr;
...
var iterateObject2replaceCertainValue = function iterateObject2replaceCertainValue(rawData, converter, options) {
...
var iterateObject2replaceCertainValueByRegex = function iterateObject2replaceCertainValueByRegex(rawData, regex, certainValue, options) {
  if (!certainValue) {
    return rawData;
  }
  ...
  var recurHandler = function recurHandler(elem, regex, certainValue) {
    if (oType(elem) !== 'object') {
      return;
    }
...
var iterateObjectHandler = function itrObj(obj, handler) {
...
var iterateObject4DiffHandler = function itrObj4Dif(obj1, obj2, customizer) {
  ...
  for (var key in obj1) {
    var subObj1 = obj1[key];
    var subObj2 = obj2[key];
    var subpaths = deepClone(paths);
    subpaths.push(key);
    var rst = itrObj4Dif(subObj1, subObj2, customizer, subpaths);
    result.push(rst);
  }
...
module.exports.formatISOdate2 = function (str) {
...
module.exports.filterNonNumString = function (str) {
  var nonRegex = new RegExp('[^\\d]', 'g'); // 替换非数字字符
...
var getTimeUnitChn = function getTimeUnitChn(str) {
  var dict = {
    ms: '毫秒',
    ss: '秒',
    mi: '分钟',
...
module.exports.filterJson = function (data, map) {
  ...
  for (var i = 0; i < map.length; i++) {
    var key = map[i];
    if (data[key] !== undefined) json[key] = data[key];else json[key] = '';
  }
...
var isNotEmptyObject = function isNotEmptyObject(obj) {
...
var setInitCap = function setInitCap(str) {
  var type = oType(str);
...
var setEachInitCap = function setEachInitCap(input) {
...
var getMultiSubString = function getMultiSubString(array, src, from, to) {
...
function cutStr(str, L) {
  ...
  for (var i = 0, j = 0; i < strlen; i++) {
    var chr = str.charAt(i); // eslint-disable-next-line no-control-regex
...
var joinEscapeCustomizer = function joinEscapeCustomizer(arr, ch) {
  var escapeChars = ['^', '$'];
  ...
  var arr2 = arr.map(function (a) {
    ...
    var regex = new RegExp(escape4RegExp(ch), 'g');
    var c = b.replace(regex, "".concat(ch).concat(ch));
    escapeChars.forEach(function (esch) {
      var regex = new RegExp(escape4RegExp(esch), 'g');
      c = c.replace(regex, "".concat(ch).concat(esch));
    });
...
var splitEscapeCustomizer = function splitEscapeCustomizer(str, ch, stage) {
  var escapeChars = ['^', '$'];
  ...
  var pushSubArr = function pushSubArr(i, ch) {
  ...
  for (var i = 0, j = 0; i < str.length; i++) {
    ...
    if (i === str.length - 1) {
      ...
      if (stage === 2) {
        var escapeCharIdx = escapeChars.indexOf(ich);
        ...
        var i2ch = str.charAt(i + 1);
        var escapeChar2Idx = escapeChars.indexOf(i2ch);
        ...
        var _i2ch = str.charAt(i + 1);

        var _escapeChar2Idx = escapeChars.indexOf(_i2ch);

        if (_i2ch === ch || _escapeChar2Idx > -1) {
          if (_i2ch === ch) {
            pushSubArr.call(arr, j, ch);
          } else {
            pushSubArr.call(arr, j, "".concat(ch).concat(_i2ch));
          }
...
var escape4RegExp = function escape4RegExp(str) {
  if (isBlank(str)) {
    return '';
  }
...
module.exports.extractImgSrc = function (str) {
  ...
  var regex = /<img[^<]+[(/>)$]/i,
      _imgStr = str.match(regex); //        console.log('imgStr', _imgStr);
  ...
  var _imgStr2 = _imgStr[0].match(regex2),
      _imgSrc = null; //        console.log('imgStr2', _imgStr2);
...
var startsWith = function startsWith(str1, startStr) {
...
var endsWith = function endsWith(str1, endStr) {
...
module.exports.indexOfChn = function (str, startIdx) {
  var str2 = oType(startIdx) === 'number' && startIdx > 0 ? str.slice(startIdx) : str;
  var matchResult = getDeepVal(str2.match(/[\u4e00-\u9fa5]/), [0]);
  var nextChnCharIdx = matchResult ? str.indexOf(matchResult, startIdx) : -1;
...
var deepClone = function fnDeepClone(obj) {
  ...
  if (obj && (0, _typeof2["default"])(obj) === 'object') {
    for (key in obj) {
      ...
      } else if (obj[key] && (0, _typeof2["default"])(obj[key]) === 'object') {
...
function parseJsonWithNumber2String(str) {
  ...
  for (var j = 0; j < regexArr.length; j++) {
    ...
    for (var i = 0; i < matchArr.length; i++) {
      ...
      var lastChar = targetStr.slice(-1);
      var last2Char = targetStr.slice(-2, -1);

      if (tgtStrlen > 2) {
        if (!/\\d/.test(lastChar) && last2Char !== '"') {
          targetStr = targetStr.slice(0, -1) + '"' + lastChar; // console.log('sourceStr:', sourceStr, 'targetStr:', targetStr, 'j:', j)
...
var getEngCharLength = function getEngCharLength(str) {
  if (!str) {
    return 0;
  }
...
var getChnCharLength = function getChnCharLength(str) {
  if (!str) {
    return 0;
  }
...
module.exports.splitDollarVarTemplate = function (template) {
  var varObj = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
  var shrinks = [];

  for (var i = 0, head = -1, tail = -1; i < template.length; i++) {
  ...
  for (var _i2 = 0, j = 0, subArr = []; _i2 < template.length; _i2++) {
    if (j % 2 === 0 && _i2 !== shrinks[j]) {
      subArr.push(template[_i2]);
    } else if (j % 2 === 0 && _i2 === shrinks[j]) {
      if (j + 1 < shrinks.length) {
        var varIdx1 = shrinks[j] + 2;
        var varIdx2 = shrinks[j + 1];
  ...
  return tempArr.filter(function (s) {
    return s;
  });
};

var iterateSplitHtmlTagTemplate = function iterateSplitHtmlTagTemplate(str, tag, results) {
  var arr = splitHtmlTagTemplate(str, tag);
...
var splitHtmlTagTemplate = function splitHtmlTagTemplate(template, tag) {
  var startTag = '<' + tag + '>';
  var startTag2 = '<' + tag + ' ';
  var endTag = '</' + tag + '>';
  var shrinks = [];

  for (var i = 0, head = -1, tail = -1, nestedTag = 0; i < template.length; i++) {
    if (template.length - i > 2 && template.slice(i, i + startTag.length) === startTag) {
      ...
      var firstLeftBracket = template.indexOf('>', i + startTag2.length);

      if (head < 0 && tail < 0) {
        head = firstLeftBracket + 1; // head = i + startTag2.length;
      } else if (head > 0 && tail < 0) {
        nestedTag++;
      }
...
function setDeepVal(data, keyArr, newValue) {
  ...
  var keyArr2 = keyArr.length > 1 ? keyArr.slice(0, keyArr.length - 1) : [];
  var lastKey = keyArr.length > 1 ? keyArr.slice(-1)[0] : keyArr[0];

  if (['undefined'].indexOf(oType(lastKey)) > -1) {
    console.error('setDeepVal ERROR - lastKey is undefined:', {
      keyArr: keyArr,
      newValue: newValue,
      data: data
    });
  ...
  var lastSecondObj = keyArr2.length > 0 ? getDeepVal(data, keyArr2) : data;
...
function getDeepVal(data, keyArr, defaultVal) {
  var keyArrCopy = deepClone(keyArr);
  var result = [];
...
var regexIndexOf = function regexIndexOf(regex, startpos) {
...
var regexLastIndexOf = function regexLastIndexOf(regex, startpos) {
  regex = regex.global ? regex : new RegExp(regex.source, "g" + (regex.ignoreCase ? "i" : "") + (regex.multiLine ? "m" : ""));
...
var getUrlParams = function getUrlParams(search) {
  var httpHeadRegExp = new RegExp('^http[s]?://');
  ...
  var RTN = {};
  var tmp = search.slice(1).split('&').map(function (a) {
    var arr = a.split('=');
...
var stepDownIfConditionSatisfiedPromise = function stepDownIfConditionSatisfiedPromise(conditionHandler, options) {
  if (!options) {
    options = {};
  }

  var recurInterval = options.recurInterval || 500;

  if (!conditionHandler || !(conditionHandler instanceof Function)) {
    return Promise.resolve(null);
  }

  var recurHandler = function recurHandler(rsv, rej, times) {
    if (!times) {
      times = 0;
    }
...
var isEndWith = function isEndWith() {
  var str = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';
  var endStr = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '';
  var idx = str.length - endStr.length;
  return idx >= 0 && str.lastIndexOf(endStr) === idx;
};

module.exports.isEndWith = isEndWith;

var getPlaceOfBirthEng = function getPlaceOfBirthEng(birthPlace) {
  var placeOfBirthEng;
  var placeOfBirthEngObj = {
    '北京': 'BEIJING',
    '天津': 'TIANJIN',
    '上海': 'SHANGHAI',
    '重庆': 'CHONGQING',
  ...
  var cityArr = Object.keys(placeOfBirthEngObj);
...
var getPlaceOfIssueEng = function getPlaceOfIssueEng(issuePlace) {
  var placeOfIssueEng;
  var placeOfIssueEngObj = {
    '北京': 'BEIJING',
    '天津': 'TIANJIN',
    '上海': 'SHANGHAI',
    '重庆': 'CHONGQING',
  ...
  var cityArr = Object.keys(placeOfIssueEngObj);
...
var getAirportCHN = function getAirportCHN(airport) {
  var airportCHN = '';
...
var hasKeyWords = function hasKeyWords(str, arr) {
  if (!str) {
    return '';
  }
...
module.exports.encodeUnicode = function (str) {
  var res = [];
...
module.exports.decodeUnicode = function (str) {
  str = str.replace(/\\/g, "%");
  return unescape(str);
};
/** 获取两个字符串的差异部分 */
...
module.exports.subtractStrings = function (a, b) {
  ...
  for (var i = 0; i < arrA.length; i++) {
    ...
    if (strA !== strB) {
      ...
      var diff = a.slice(cursor, cursor + offset2);
      cursorArr.push({
        cursor: cursor,
        offset: offset,
        diff: diff
...
//# sourceMappingURL=hdlUtil.js.map
